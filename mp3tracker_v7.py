...
class MP3TrackerApp(ctk.CTk):
    ...
    def open_pack_manager(self):
        ...
        auto_clr = CLR_ACCENT if HAS_MUTAGEN else CLR_MUTED
        auto_tip = "🎨  Авто по обложке" if HAS_MUTAGEN else "pip install mutagen"
        ctk.CTkButton(hdr, text=auto_tip, height=34, corner_radius=8,
                      fg_color=auto_clr, hover_color=CLR_ACCENT2 if HAS_MUTAGEN else CLR_MUTED,
                      font=ctk.CTkFont(size=12, weight="bold"),
        ...
        def refresh_pack_list():
            ...
            for pid, pack in self.packs.items():
                ...
                dot_c = tk.Canvas(row, width=10, height=10,
    ...
    def open_pack_dashboard(self):
        ...
        ctk.CTkLabel(
            win,
            text="Строки = паки  ·  Столбцы = артисты  ·  ✓ зелёный = отправлен  ·  — серый = не отправлен",
        ...
        # Header row
        tk.Label(grid_frame, text="", width=28,
                 bg=CLR_CARD, fg=CLR_MUTED, font=FONT_SM,
                 relief="flat", padx=4, pady=6).grid(row=0, column=0, padx=1, pady=1, sticky="nsew")

        for col, (_, data) in enumerate(artists, start=1):
            name  = data['display']
            short = name if len(name) < 14 else name[:12] + "…"
            tk.Label(grid_frame, text=short, width=14,
                     bg=CLR_CARD, fg=CLR_CYAN, font=FONT_SM,
                     relief="flat", padx=4, pady=6, anchor="center"
                     ).grid(row=0, column=col, padx=1, pady=1, sticky="nsew")

        # Pack rows
        for row, (pid, pack) in enumerate(packs, start=1):
            color  = pack.get("color", CLR_ACCENT)
            pname  = pack['name']
            short_p = pname if len(pname) < 24 else pname[:22] + "…"
            row_bg  = CLR_CARD if row % 2 == 0 else "#10112A"

            # Pack name cell
            cell_f = tk.Frame(grid_frame, bg=row_bg, width=200)
            cell_f.grid(row=row, column=0, padx=1, pady=1, sticky="nsew")
            cell_f.pack_propagate(False)
            tk.Frame(cell_f, bg=color, width=5).pack(side="left", fill="y")
            tk.Label(cell_f, text=short_p, bg=row_bg, fg=CLR_TEXT,
                     font=FONT_SM, anchor="w", padx=6).pack(
                side="left", fill="both", expand=True)

            # Artist cells
            for col, (raw, _) in enumerate(artists, start=1):
    ...
    def refresh_chats_ui(self):
        ...
        for raw, data in self.chats.items():
            ...
            badge  = f"  📦{unsent}" if unsent > 0 else ""

            ctk.CTkButton(
                f, text=data['display'].upper() + badge, anchor="w",
